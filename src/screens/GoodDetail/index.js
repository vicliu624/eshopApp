...
export default class extends React.Component {
  static navigationOptions = ({navigation}) => ({
    header: () => {
      const backToHome = navigation.state.params ?
        ...
        : true
      return (
        <HeaderDefault title="商品详请" backToHome={backToHome} navigation={navigation} />
      )
    }
  })
  ...
  render() {
    ...
    const categoryName = good ? (
      good.category ? good.category.name : ""
    ) : ""
            ...
            !inService ? (
              <View style={{flex: 1}}>
                ...
                <View style={styles.titleWrapper}>
                  <Text style={styles.title} numberOfLines={2}>{good.goodsName}</Text>
