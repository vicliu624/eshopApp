...
export default class Cart extends React.Component {
  static navigationOptions = ({navigation}) => ({
    header: () => {
      const backToHome = navigation.state.params ?
        ...
        : true
      return (
        <HeaderDefault title="购物车" backToHome={backToHome} navigation={navigation} />
      )
    }
  })
  ...
  _handleGoodDelete = async (value) => {
    ...
    console.log('_handleGoodDelete--------->',value)
    if (isAuthorized) {
      await this.props.addCart(userId, token, value.good.id, -value.count)
  ...
  _handleGoodCountChange = async (value, count) => {
    ...
    if (isAuthorized) {
      ...
      if (count >= 0) {
        await this.props.addCart(userId, token, value.good.id, 1)
        let cartStata = cartState.map((item) => {
  ...
  _handleOrder = () => {
    ...
    let address = this.checkDefaultAddress()
    if(!address){
      this._showToast('请先添加一个默认收货地址')
      return
    }
            ...
            !isAuthorized ? (
              ...
              <View>
                <View style={styles.contentHeader}>
                  <CheckBox value={cartChecked} color={RED_COLOR} onChange={this._handleSelectAll}/>
